    ...
    >
        ...
        <div className="flex items-center gap-3 shrink-0">
          {hasActivity && (
            <span className="text-xs hidden sm:inline" style={{ color: "#b07a70" }}>
              🏃 {t.activityLabels?.[record.physicalActivity] ?? `${record.physicalActivity}min`}
            </span>
          )}
          {score > 1 && (
            <span
      ...
      {expanded && (
        ...
        >
              ...
              {SYMPTOM_FIELDS.map(({ key, labelKey, fallback }) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-2">
                    <span className="text-xs shrink-0" style={{ color: "#b07a70", width: 130 }}>
                      {t[labelKey] ?? fallback}
                    </span>
                    <ScoreBar value={val} />
                  </div>
          ...
          {/* Activity & sleep */}
          {(hasActivity || record.sleepHours > 0) && (
            <div className="flex gap-2">
              ...
              {record.sleepHours > 0 && (
                ...
                >
                  <p className="text-xs" style={{ color: "#7b68ee" }}>{t.symptomSleep ?? "Sleep"}</p>
