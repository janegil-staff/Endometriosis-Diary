...
export function OverviewCard({ t, records }) {
  ...
  const avgPain = painScores.length
    ? Math.round(
        (painScores.reduce((a, b) => a + b, 0) / painScores.length) * 10,
      ) / 10
    : null;
  ...
  const sleepPoor = records.filter(
    (r) => (r.sleepQuality ?? 0) === 1 && (r.sleepHours ?? 0) > 0,
  ).length;
  ...
  const absentSocialPartial = records.filter(
    (r) => r.absentSocial === 2,
  ).length;
  ...
  const stats = [
    ...
    {
      label: (t.absentWork ?? "Absent work") + ` (${t.full ?? "full"})`,
      value: absentWorkFull,
      color: absentWorkFull > 0 ? "#FF7473" : "#4CC189",
    },
    {
      label: (t.absentSocial ?? "Absent social") + ` (${t.partial ?? "part"})`,
      value: absentSocialPartial,
      color: absentSocialPartial > 0 ? "#FFC659" : "#4CC189",
    },
    {
      label: (t.absentSocial ?? "Absent social") + ` (${t.full ?? "full"})`,
