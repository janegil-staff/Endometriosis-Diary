...
// ── Pain Overview Card (replaces CatOverviewCard) ──────────────────────────
export function PainOverviewCard({ t, avgPain, minPain, maxPain, flareUps, periodDays, recordsCount }) {
  return (
    <Card
      title={t.painScore ?? "Pain score"}
      accent={avgPain != null ? { value: avgPain, color: PAIN_COLOR(avgPain) } : undefined}
      subtitle={t.avgSymptoms ?? "Avg. symptoms"}
    >
      <StatRow label={t.daysRecorded ?? "Days recorded"} value={recordsCount} />
      {minPain != null && (
        <StatRow label="Min" value={minPain} color={PAIN_COLOR(minPain)} />
      )}
      {maxPain != null && (
        <StatRow label="Max" value={maxPain} color={PAIN_COLOR(maxPain)} />
      )}
      <StatRow
        label={t.exacerbation ?? "Flare-ups"}
        value={flareUps}
        color={flareUps > 0 ? "#f5a623" : "#5cb85c"}
      />
      <StatRow
