...
const SYMPTOM_FIELDS_PDF = [
  ...
  {
    key: "bowelMovementPain",
    labelKey: "symptomBowel",
    fallback: "Bowel pain",
  },
...
function Toggle({ checked, onChange, label, color = "#c97060" }) {
  return (
    <label className="flex items-center gap-3 cursor-pointer select-none">
      ...
      <span
        className="text-sm"
        style={{ color: checked ? "#5a3a34" : "#b07a70" }}
      >
        {label}
      </span>
    </label>
...
export default function PdfExportModal({ open, onClose, patient, t }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setFont(18, "bold", ink);
      doc.text(
        (t.reportTitle ?? t.symptomLog ?? "Symptom Report").toUpperCase(),
        ML,
        26,
      );
      ...
      doc.text(`${patient.age} · ${t.female ?? "Female"}`, W - MR, 30, {
        align: "right",
      });
      ...
      const avgPain = scores.length
        ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length)
        : null;
      const flareCount = filtered.filter(
        (r) => r.intensity >= 4 || r.bowelMovementPain >= 4 || r.endoBelly >= 4,
      ).length;
      const periodDays = filtered.filter((r) => r.period >= 2).length;
      const medDays = filtered.filter(
        (r) => r.acuteMedicines?.length > 0,
      ).length;
      ...
      // ── Column layout ───────────────────────────────────────
      // Pain score wider (18), medicine narrower (28/40), misc fills remainder
      ...
      colX += 29;
      if (fields.painScore) {
        COL.pain = { x: colX, w: 22 };
        colX += 23;
      }
      if (showSymptoms) {
        COL.syms = { x: colX, w: showMeds ? 44 : 64 };
        colX += COL.syms.w + 1;
      }
      if (showMeds) {
        COL.meds = { x: colX, w: showMisc ? 28 : 40 };
        colX += COL.meds.w + 1;
      }
      if (showMisc) {
        COL.misc = { x: colX, w: W - MR - colX };
      }
      // ── Table header ────────────────────────────────────────
      checkY(10);
      const thH = 7;
      ...
      if (COL.pain)
        doc.text(
          (t.painScore ?? "Pain").toUpperCase(),
          COL.pain.x + COL.pain.w / 2,
          y + 4.8,
          { align: "center" },
        );
      if (COL.syms)
        doc.text(
          (t.symptomLog ?? "Symptoms").toUpperCase().slice(0, 18),
          COL.syms.x + 2,
          y + 4.8,
        );
      if (COL.meds)
        doc.text(
          (t.medication ?? "Medicine").toUpperCase(),
          COL.meds.x + 2,
          y + 4.8,
        );
      if (COL.misc) {
        doc.setFontSize(5.5);
        doc.text(
          `${t.physicalActivity ?? "Activity"} / ${t.symptomSleep ?? "Sleep"}`
            .toUpperCase()
            .slice(0, 40),
          COL.misc.x + 2,
          y + 4.8,
      ...
      filtered
        ...
        .forEach((r, idx) => {
          ...
          const prevYear =
            idx > 0 ? filtered[filtered.length - idx].date.slice(0, 4) : null;
          if (year !== prevYear) {
            checkY(10);
            if (idx > 0) y += 3;
          ...
          const isFlareUp =
            fields.flareUp &&
            (r.intensity >= 4 || r.bowelMovementPain >= 4 || r.endoBelly >= 4);
          ...
          if (COL.pain && score > 1) {
            setFont(13, "bold", ink);
            doc.text(String(score), COL.pain.x + COL.pain.w / 2, ty + 2, {
              align: "center",
            });
          ...
          if (COL.syms && showSymptoms) {
            ...
            visibleSymptoms
              .slice(0, 10)
              .forEach(({ key, labelKey, fallback }, i) => {
                const col = i % 2;
          ...
          if (COL.misc) {
            ...
            if (fields.activity && r.physicalActivity > 0) {
              doc.text(
                t.activityLabels?.[r.physicalActivity] ??
      ...
      doc.save(
        `${t.reportTitle ?? "endo-diary"}-${fromDate ?? ""}-${toDate ?? ""}.pdf`,
      );
      onClose();
    } catch (err) {
  ...
  const fieldGroups = [
    {
      ...
      label: t.painScore ?? "Pain score",
      children: [
        { key: "symptoms", label: t.symptomLog ?? "All symptom fields" },
      ],
    },
